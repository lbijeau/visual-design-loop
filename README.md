...
| 1. Lock the tokens | 2. First draft | 3. Regression caught | 4. Converged |
| :---: | :---: | :---: | :---: |
| ![Style tile](./assets/shot1_style_tile.png) | ![Initial draft](./assets/shot2_initial_draft.png) | ![Regression detected](./assets/shot3_regression.png) | ![Converged design](./assets/shot4_convergence.png) |
| Brand tokens are approved via a style tile before any UI code is written. | A high-fidelity draft is generated, strictly constrained to the tokens. | A token tweak introduces a contrast failure — the auditor flags it immediately. | The corrective loop stabilizes the tokens and the layout converges. |
...
- **Theme approval gate.** Before any UI code is written, StyleSentry generates a *style tile*. You approve it (`APPROVE THEME`) or tweak it in natural language ("make the accent a deep neon purple") and the tokens update instantly.
...
The same orchestrator produces radically different looks — the token lock keeps each one internally consistent and accessible. All three were generated by the identical loop, differing only in the prompt:
...
1. **Approve the theme.** Review the style tile; type `APPROVE THEME`, or describe a change to adjust the tokens.
2. **Iterate.** Give layout or theme feedback in plain language ("move the nav left, add hero padding"; "lighten the primary 10%"). Theme changes run through a corrective validation gate — if a token edit drops the audit score by more than 15 points or introduces a critical bug, the tokens are corrected *before* any HTML is touched.
3. **Finish.** `✓ Accept & Finish` (or `DONE`) exports the self-contained `final_result.html`. `↩ Undo` (or `UNDO`) steps back one iteration, repeatable to the first draft.
